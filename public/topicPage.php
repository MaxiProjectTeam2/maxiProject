...
<body>
  ...
  <div id="navigation">
  	<a href="patientLogin.php"><button class="btn btn-primary col-lg-2 col-lg-offset-10">Exit</button></a>
  </div>
      ...
      $contentA = $db->fetch_assoc($content1);
			...
			echo "<label class=\"control-label sr-only col-lg-offset-2 col-lg-2\" for=\"topic\">Topic1:</label>";
			echo "<div class=\"col-lg-6\">";
			echo "<a href=\"topicPage.php?contentID=" . $contentA['ID'] . "&amp;patientID=" . $_GET["patientID"] . "\" name=\"topic\"><button class=\"btn btn-default col-lg-8\">" . $subTopicA['subTopic1'] . "</button></a>";
			echo "</div>";
			echo "</div>";
        ...
        $contentB = $db->fetch_assoc($content2);
			echo "<div class=\"form-group\">";
			echo "<label class=\"control-label sr-only col-lg-offset-2 col-lg-2\" for=\"topic2\">Topic2:</label>";
			echo "<div class=\"col-lg-6\">";
  			echo "<a href=\"topicPage.php?contentID=" . $contentB['ID'] . "&amp;patientID=" . $_GET["patientID"] . "\" name=\"topic2\"><button class=\"btn btn-default col-lg-8\">" . $subTopicB['subTopic2'] . "</button></a>";
			echo "</div>";
			echo "</div>";
			}
        ...
        $contentC = $db->fetch_assoc($content3);
			echo "<div class=\"form-group\">";
			echo "<label class=\"control-label sr-only col-lg-offset-2 col-lg-2\" for=\"topic3\">Topic2:</label>";
			echo "<div class=\"col-lg-6\">";
  			echo "<a href=\"topicPage.php?contentID=" . $contentC['ID'] . "&amp;patientID=" . $_GET["patientID"] . "\" name=\"topic3\"><button class=\"btn btn-default col-lg-8\">" . $subTopicC['subTopic3'] . "</button></a>";
			echo "</div>";
			echo "</div>";
			}
        ...
        $contentD = $db->fetch_assoc($content4);
			echo "<div class=\"form-group\">";
			echo "<label class=\"control-label sr-only col-lg-offset-2 col-lg-2\" for=\"topic4\">Topic2:</label>";
			echo "<div class=\"col-lg-6\">";
  			echo "<a href=\"topicPage.php?contentID=" . $contentD['ID'] . "&amp;patientID=" . $_GET["patientID"] . "\" name=\"topic4\"><button class=\"btn btn-default col-lg-8\">" . $subTopicD['subTopic4'] . "</button></a>";
			echo "</div>";
			echo "</div>";
        ...
        $contentE = $db->fetch_assoc($content5);
			...
			echo "<label class=\"control-label sr-only col-lg-offset-2 col-lg-2\" for=\"topic4\">Topic2:</label>";
			echo "<div class=\"col-lg-6\">";
  			echo "<a href=\"topicPage.php?contentID=" . $contentE['ID'] . "&amp;patientID=" . $_GET["patientID"] . "\"><button class=\"btn btn-default col-lg-8\">" . $subTopicE['subTopic5'] . "</button></a>";
			echo "</div>";
			echo "</div>";
			}
    ...
    <?php
      if(isset($_GET["contentID"])){
		...
		if(isset($_GET["patientID"])&&isset($_GET["contentID"])){
		//$con = new mysqli("localhost","admin","[J%fLP+o]","rtooldb");
		$patientID=$_GET["patientID"];
		$contentID=$_GET["contentID"];
		$viewID = "SELECT MAX(ViewId) as max FROM viewhistory where contentID={$contentID} and patientID={$patientID}";
		$result = $db->query($viewID);
		while($row = $db->fetch_assoc($result)){
			$viewId = $row["max"];
			$viewId = $viewId + 1;
		}
		echo "<script>var patientID = {$patientID};
		var contentID = {$contentID};
		var viewId = {$viewId};</script>";
		$sql = "INSERT INTO `viewhistory` (`patientID`, `contentID`, `viewId`, `startTime`) VALUES ({$patientID}, {$contentID}, {$viewId}, current_time());";
		$db->query($sql);	
		echo "<script>
		window.onunload = window.onbeforeunload = (function(){
		    $.post(\"track.php\",{patientID:patientID,contentID:contentID,viewId:viewId});
		});
		</script>";}
	?>
		<div class="form-horizontal">
			<div class="form-group">
				...
				<?php
				  $getPath = "SELECT path,contentTypeID from content WHERE contentID={$contentID}";
				  ...
				  if($path['contentTypeID'] == 1){
					...
					<div id="picturePlayer">
					  <img src="<?php echo $path['path']?>" alt="Mountain View" style="width:100%;height:100%; margin:0 auto "">
					</div>
				  ...
				  elseif($path['contentTypeID'] ==2){
					...
					<div id="audioplayer">
					  <video width="640" height="360" controls>
						<source src="<?php echo $path['path']?>" type="video/mp4" />
